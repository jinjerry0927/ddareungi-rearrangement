...
- 문서 버전: 1.0
- 작성일: 2026-08-19
- 현재 상태: M9 donor reserve 홀드아웃·포트폴리오 정리 완료, 차량 실행모델 설계
...
잠재수요 v2는 시간별 재고의 0대 경계와 학습기간 비품절 대여율을 결합한 Poisson 민감도로
동결했다. 평가 정책의 시뮬레이션 재고가 아니라 정책 실행 전에 만든 공통 요청열을 사용하며,
세 수준은 `low ⊆ base ⊆ high`로 중첩된다. ID·강도 backoff·p95 강도 경계·Poisson q99 상한·
50개 공통 seed·
합성 반납·자전거 보존식·누수 방지 규칙은
[잠재수요 생성 계약](LATENT_DEMAND_CONTRACT.md)을 따른다.

2026-08-31 기준 v2 manifest 생성과 165개 대여소 품질 게이트, 선택적 `ReplayScenario`
통합, 50개 seed × 세 수준 × P0/P2-R 300개 정책 실행과 대여소 공간 강건성까지 완료했다.
모든 사전 중단조건과 450개 대여소 재조정이 통과했으며 세 수준 공통 persistent 악화 7곳을
분리했다. 다음은 정책을 바꾸기 전에 이 지점들의 재배치 유출·빈 시간·요청 구성을 진단한다.
